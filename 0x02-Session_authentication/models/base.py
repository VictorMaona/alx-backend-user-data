...
class Base():
    ...
    def to_json(self, for_serialization: bool = False) -> dict:
        """ Transform the item into JSON dictionary
        """
        result = {}
    ...
    def load_from_file(cls):
        """ all objects in file to be loaded
        """
    ...
    def save_to_file(cls):
        """ all objects in file to ba saved
        """
        s_class = cls.__name__
        file_path = ".db_{}.json".format(s_class)
        objs_json = {}
    ...
    def search(cls, attributes: dict = {}) -> List[TypeVar('Base')]:
        """ Objects searched with matching attributes
        """
        s_class = cls.__name__
        def _search(obj):
            if len(attributes) == 0:
